    ...
    <div class="row justify-content-center">
        <div class="col-md-12">
			<div class="modal-header">
				...
				<a class="btn-link link" href="{{route('student.job.index')}}">
					<button class="mt-1 btn btn-sm btn-primary">All Jobs</button>
				</a>
			</div>
			@if(session('success'))
			...
			<div class="modal-body">
				<div class="row container">
					<div class="form-group col-md-6">
						...
						<div id="stipend">&#8377;&nbsp;{{$job->stipend}}</div>
					</div>
					<div class="form-group col-md-6">
						<label for="minPointer">Min Pointer :</label>
						<div id="minPointer">{{$job->minPointer}}</div>
					</div>
				</div>
				<div class="row container">
				...
				<div class="row">
					<div class="col-md-6 form-group">
						<button class="btn btn-sm btn-primary" id="applyJobBtn">Apply</button>
					</div>
				</div>
			</div>
        </div>
...
// alert(sem6Cgpa);
	...
	var minPointer = document.getElementById('minPointer').innerHTML;
	//alert(minPointer);
	var studentCgpa = document.getElementById('avgCGPA').innerHTML;
	//alert(studentCgpa);
	
	if(studentCgpa > minPointer){
		document.getElementById('applyJobBtn').enabled = true;
		swal("You, can Apply.!!", {icon: "success", buttons: false, timer: 3000,});
		//alert('You are Eligible');
	}else{
		document.getElementById('applyJobBtn').disabled = true;
		swal("Sorry, you can't Apply.!!", {icon: "error", buttons: false, timer: 3000,});
		//alert('Sorry');
	}
